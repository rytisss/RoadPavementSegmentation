...
def render_video(images_dir, label_dir, prediction_dir, output_video, title):
    image_paths = gather_image_from_dir(images_dir)
    ...
    architectureVideoName = output_video
    ...
    for i in range(0, len(image_paths)):
        ...
        # resize image to fit in 480 width
        original_image_height, original_image_width = get_image_width_n_height(image_with_prediction)
        resize_ratio = float(original_image_width) / float(desired_width)
        output_width = desired_width
        output_height = (int)(original_image_height / resize_ratio)

        image_with_label = cv2.resize(image_with_label, (output_width, output_height))
        label_color = cv2.resize(label_color, (output_width, output_height))
        prediction_color = cv2.resize(prediction_color, (output_width, output_height))
        image_with_prediction = cv2.resize(image_with_prediction, (output_width, output_height))
...
def main():
    image_dir = r'D:\pavement defect data\GAPs384_output_0.5percent_size\Test\Images/'
    label_dir = r'D:\pavement defect data\GAPs384_output_0.5percent_size\Test\Labels/'
    prediction_dir = r'D:\pavement_defect_results\pretrained_Unet4_res_asppWF_AG\gaps384\gaps384_pretrained_UNet4_res_asppWF_AG_21750/'
    title = 'GAPs384 ResUNet+ASPP_WF+AG'
    video_out = r'C:\Users\Rytis\Desktop\Straipsniai\Sensors_FromIDAACS_v2\video/' + title + '.avi'
    render_video(image_dir, label_dir, prediction_dir, video_out, title)
